...
# HTTPS Setup Script for Amazon Linux 2023
# Amazon Linux 2023 doesn't use EPEL, Certbot is available directly

set -e

echo "=========================================="
echo "🔒 HTTPS/SSL Setup for Wash Booking"
echo "   (Amazon Linux 2023 Version)"
echo "=========================================="
echo ""

# Check if running as root
if [ "$EUID" -ne 0 ]; then 
    echo "❌ Please run as root (sudo ./setup-https-al2023.sh)"
    exit 1
fi
...
dnf install -y python3-certbot-nginx > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "✅ Certbot installed"
else
    echo "⚠️  Trying alternative installation method..."
    dnf install -y python3-pip > /dev/null 2>&1
    pip3 install certbot certbot-nginx > /dev/null 2>&1
...
# Step 6: Configure firewall (if firewalld is running)
...
# Step 7: Check DNS before proceeding
...
if [ "$CURRENT_IP" != "$DOMAIN_IP" ]; then
    echo ""
    echo "⚠️  WARNING: Domain DNS may not be configured correctly!"
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Common issues:"
    echo "  1. Domain DNS not pointing to this server ($CURRENT_IP)"
    echo "  2. Ports 80 and 443 not open in AWS Security Group"
    echo "  3. Domain not accessible via HTTP"
    echo ""
    echo "To debug:"
...
echo "Step 9: Testing certificate auto-renewal..."
...
echo "Step 11: Final Nginx restart..."
systemctl restart nginx
